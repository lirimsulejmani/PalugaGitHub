"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var account_service_1 = require("./account.service");
var school_service_1 = require("../school/school.service");
var student_model_1 = require("./student.model");
var SignUpComponent = (function () {
    function SignUpComponent(accountService, schoolService, router) {
        this.accountService = accountService;
        this.schoolService = schoolService;
        this.router = router;
        this.student = new student_model_1.Student();
    }
    SignUpComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.schoolService.getSchools().subscribe(function (res) { return _this.schools = res.data; });
    };
    SignUpComponent.prototype.onSubmit = function (form) {
        var _this = this;
        debugger;
        this.accountService.signUp(this.student)
            .subscribe(function (res) {
            _this.student = res.data;
            _this.router.navigate(['/confirm-signup']);
        }, function (error) {
            debugger;
            console.log(error);
            _this.errorMessage = JSON.parse(error._body).error;
        });
    };
    return SignUpComponent;
}());
SignUpComponent = __decorate([
    core_1.Component({
        selector: 'signup-container',
        templateUrl: "./signup.view.html?v=" + Date()
    }),
    __metadata("design:paramtypes", [account_service_1.AccountService, school_service_1.SchoolService, router_1.Router])
], SignUpComponent);
exports.SignUpComponent = SignUpComponent;