...
using ServiceLayer.APIs.Goodreads.CSV;
...
namespace Goodreads.Models.Response
{
    ...
    public class Book : ApiResponse
    {
        ...
        //added fields not from API 
        public string AuthorName { get; set; }

        public string LargeImageUrl {
            get {
                var img = "";
                var last = ImageUrl.Split('/').Last();
                ImageUrl.Split('/').ToList().ForEach(t => {
                    if (!t.EndsWith(".com") && t.EndsWith("m"))
                    {
                        img += t.Replace("m", "l") + "/";
                    }
                    else
        ...
        public string Language {
            get {
                return CsvProcessor.GetLanguageName(LanguageCode);
            }
        }
