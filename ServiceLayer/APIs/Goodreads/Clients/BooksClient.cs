...
namespace Goodreads.Clients
{
    ...
    public class BooksClient : IBooksClient
    {
        ...
        public async Task<IReadOnlyList<int?>> GetWorkIdsForBookIds(IReadOnlyList<int> bookIds)
        {
            ...
            if (response != null && (int)response.StatusCode >= 200 && (int)response.StatusCode < 300)
            {
                ...
                if (!string.IsNullOrWhiteSpace(content))
                {
                    ...
                    var items = document.Elements("GoodreadsResponse/work-ids/item");
        ...
        //    var response = await Connection.Client.ExecuteAsyncGet<ReviewStatsContainer>(request, null, "GET").ConfigureAwait(false);
